...
import { BookOpen, Check, ChevronRight, Star } from "lucide-react";
import type { Book } from "../features/books/types";
import {
  SHELF_BOOK_DRAG_MIME,
  readShelfBookDrag,
  writeShelfBookDrag,
} from "../utils/shelfDrag";
import {
  BookActionsMenu,
...
type Props = {
  ...
  /** Move books into a folder (null = uncategorized) via drag-and-drop onto a section */
  onMoveBooksToFolder: (folderId: string | null, bookIds: string[]) => void;
};
...
function BookCard({
  ...
  return (
    <article
      draggable
      onDragStart={(event) => {
        const ids = selectedIds.includes(book.id) ? selectedIds : [book.id];
    ...
    >
      ...
      <div className="relative mx-auto h-[210px] w-[140px]">
        <div className={`book3d ${status === "reading" ? "is-reading" : ""}`}>
      ...
      <div className="mx-auto mt-2 flex w-[140px] items-center justify-between">
        <div className="flex items-center gap-1.5">
          <div
            ...
            aria-label={book.rating ? `Rated ${book.rating} of 5` : "Not rated"}
          >
            {[1, 2, 3, 4, 5].map((star) => (
              <Star
                key={star}
...
export function GalleryView({
  ...
  const uncategorized = books.filter(
    (book) => book.folderId == null || book.folderId === "",
  );
  const byFolder = new Map<string, Book[]>();
  for (const book of books) {
    if (book.folderId == null || book.folderId === "") continue;
    const list = byFolder.get(book.folderId);
    if (list) list.push(book);
    else byFolder.set(book.folderId, [book]);
  }
  ...
  return (
    <div className="shelf-scroll shelf-scroll-gutter h-full min-h-0 overflow-auto px-4 py-5 sm:px-6">
      {/* Uncategorized — always shown, no section header */}
      ...
      {folderOptions.map((folder) => {
        const folderBooks = byFolder.get(folder.id) ?? [];
        const isEmpty = folderBooks.length === 0;
        ...
        return (
          <section
            ...
            className={`mt-6 first:mt-0 rounded-lg p-2 transition ${
              isDropTarget
                ? "bg-amber-50 ring-2 ring-amber-300 dark:bg-amber-400/10 dark:ring-amber-500/50"
                : ""
            }`}
            onDragOver={(event) => {
              if (!event.dataTransfer.types.includes(SHELF_BOOK_DRAG_MIME))
                return;
            ...
            onDrop={(event) => {
              event.preventDefault();
              setDropTarget(null);
              const ids = readShelfBookDrag(event.dataTransfer);
              if (ids.length) onMoveBooksToFolder(folder.id, ids);
            }}
          >
            <button
              type="button"
            ...
            {isEmpty ? (
              <p
                className={`rounded-md border border-dashed px-3 py-4 text-center text-xs transition ${
                  isDropTarget
              ...
              >
                Drop books here to add them to {folder.name}.
              </p>
            ) : isCollapsed ? null : (
              <BookGrid>
